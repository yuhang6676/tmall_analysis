...
def echarts_line(x, y, title='主标题', subtitle='副标题', label='图例'):
    ...
    line.add_yaxis(
        ...
        linestyle_opts=opts.LineStyleOpts(color='#ff0000', width=3),  # 红色线条
...
# 2. 柱状图函数：白底、红色渐变圆角柱、标记最大/最小/平均值
...
# 2. 3D地图函数（手动维护各省份经纬度坐标，低灵敏度方便微调）
def map3d_with_bar3d(province, data_list, title, label):
    ...
    map_3d.add(
        ...
        bar_size=1.2,
        itemstyle_opts=opts.ItemStyleOpts(color="#ff0000"),  # 红色3D柱
